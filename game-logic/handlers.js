...
function checkRateLimit(key, delay) {
    const now = Date.now();
...
function updateTeamMembers(io, team, roomId) {
    ...
    // Also broadcast to all to update lists
...
function productionClick(io, socket, data, room, roomId) {
    if (!checkRateLimit(socket.id, 100)) {
      return socket.emit('error', { message: '너무 빠르게 클릭하고 있습니다.' });
    }
    ...
    if (team.clickCount < config.maxClicks) {
      team.clickCount++;
      team.totalPA += config.paPerClick;
...
function disconnect(io, socket, room, roomId) {
    try {
      ...
      if (Object.keys(room.players).length === 0 && !room.adminSocketId) {
        // stopTimer(roomId); // 타이머 로직은 별도 관리 필요
        delete rooms[roomId];
