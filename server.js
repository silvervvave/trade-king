...
async function processQueue(roomId) {
  ...
  try {
    ...
    logger.error(`Error processing event for room ${roomId}:`, error);
  } finally {
    if (roomQueues[roomId]) {
...
io.on('connection', (socket) => {
  ...
  socket.on('login_or_register', async (data) => {
    ...
    const { newUserCreated } = await playerHandlers.loginOrRegister(socket, data, supabase);
    if (newUserCreated) {
  ...
  socket.on('get_room_info', (data) => {
    ...
    playerHandlers.getRoomInfo(io, socket, data, redisClient);
  });
  ...
  socket.on('join_game', (data) => {
    ...
    const task = async () => {
      try {
        await playerHandlers.joinGame(io, socket, data, redisClient, supabase);
      } catch (error) {
        logger.error(`[오류] joinGame 처리 중 오류 (roomId: ${roomId}):`, error);
  ...
  socket.on('join_super_admin_room', () => {
    ...
    logger.info(`[Super Admin] 소켓 ${socket.id}이(가) ${SUPER_ADMIN_ROOM}에 참가했습니다.`);
  });
  ...
  socket.on('delete_user', async (data) => {
    ...
    await playerHandlers.deleteUser(socket, data, supabase);
  ...
  socket.on('delete_multiple_users', async (data) => {
    ...
    await playerHandlers.deleteMultipleUsers(socket, data, supabase);
    // 사용자 삭제 후, 모든 super-admin에게 목록 업데이트
    broadcastUserListUpdate(io);
  });
  ...
  const safeHandler = async (io, socket, handler, eventName, data, callback) => {
    ...
    const task = async () => {
      try {
        ...
        logger.error(`이벤트 처리 중 오류 (roomId: ${roomId}): ${error.message}`, error.stack);
  ...
  socket.on('create_room', async () => {
    try {
      ...
      logger.info(`[방 생성] ${roomId} (관리자: ${socket.id}) - Supabase 및 Redis에 저장됨`);
      socket.emit('room_created', { roomId });

      // 4. Super-admin에게 방 목록 업데이트
      broadcastRoomListUpdate(io);
    } catch (error) {
      logger.error('방 생성 중 오류', error);
  ...
  socket.on('force_close_room', async (data) => {
    ...
    try {
      ...
      logger.warn(`[방 강제종료] ${roomId} 방이 슈퍼 관리자에 의해 삭제되었습니다.`);
      ...
      logger.error(`방 강제 종료 중 오류 (roomId: ${roomId}):`, error);
    ...
    const task = async () => {
      try {
        ...
        if (gameState.adminSocketId === null || !io.sockets.sockets.get(gameState.adminSocketId)) {
          ...
          socket.join(roomId);
          socket.roomId = roomId;
          logger.info(`[관리자 재확보] 방 ${roomId} (새 관리자: ${socket.id})`);
        ...
        logger.error(`관리자 재확보 중 오류 (roomId: ${roomId}):`, error);
  ...
  socket.on('disconnect', () => {
    ...
    const task = async () => {
      try {
        const gameStateJSON = await redisClient.get(`room:${roomId}`);
        if (!gameStateJSON) {
          logger.warn(`[연결 해제] Redis에 방 ${roomId}이(가) 없어 스킵합니다.`);
          delete roomQueues[roomId];
          return;
        }
        let gameState = JSON.parse(gameStateJSON);

        const roomWasDeleted = await playerHandlers.disconnect(io, socket, gameState, roomId, supabase);

        if (roomWasDeleted) {
          timerManager.stop(roomId, gameState);
          // No need to delete from Redis/Supabase here, disconnect handler does it.
          delete roomQueues[roomId];
          logger.info(`[메모리 정리] 빈 방 ${roomId}의 상태를 Redis에서 삭제했습니다.`);
          broadcastRoomListUpdate(io); // 방이 삭제되었으므로 목록 업데이트
        } else {
          // 상태가 변경되었으므로 다시 저장
          await redisClient.set(`room:${roomId}`, JSON.stringify(gameState));
        }
        logger.info(`[연결 해제] 소켓 ${socket.id} 처리 완료 (방: ${roomId})`);
      } catch (error) {
        logger.error(`연결 해제 처리 중 오류 (roomId: ${roomId}):`, error);
      }
    };
  ...
  // 핸들러 등록
  socket.on('register_player', (data, callback) => safeHandler(io, socket, playerHandlers.registerPlayer, 'register_player', data, callback));
  socket.on('start_phase', (data, callback) => safeHandler(io, socket, adminHandlers.startPhase, 'start_phase', data, callback));
  socket.on('complete_production_batch', (data, callback) => safeHandler(io, socket, productionHandlers.completeProductionBatch, 'complete_production_batch', data, callback));
  socket.on('trade_selection', (data, callback) => safeHandler(io, socket, tradeHandlers.tradeSelection, 'trade_selection', data, callback));
  socket.on('make_investment', (data, callback) => safeHandler(io, socket, tradeHandlers.makeInvestment, 'make_investment', data, callback));
  socket.on('play_rps', (data, callback) => safeHandler(io, socket, productionHandlers.playRPS, 'play_rps', data, callback));
  socket.on('reroll_rps', (data, callback) => safeHandler(io, socket, productionHandlers.rerollRPS, 'reroll_rps', data, callback));
  socket.on('draw_event', (data, callback) => safeHandler(io, socket, eventHandlers.drawEvent, 'draw_event', data, callback));
  socket.on('play_final_rps', (data, callback) => safeHandler(io, socket, eventHandlers.playFinalRPS, 'play_final_rps', data, callback));
  socket.on('reroll_final_rps', (data, callback) => safeHandler(io, socket, eventHandlers.rerollFinalRPS, 'reroll_final_rps', data, callback));
  socket.on('reset_game', (data, callback) => safeHandler(io, socket, adminHandlers.resetGame, 'reset_game', data, callback));
  socket.on('end_game', (data, callback) => safeHandler(io, socket, adminHandlers.endGame, 'end_game', data, callback));
  socket.on('reset_trade', (data, callback) => safeHandler(io, socket, tradeHandlers.resetTrade, 'reset_trade', data, callback));
  socket.on('reset_investments', (data, callback) => safeHandler(io, socket, tradeHandlers.resetInvestments, 'reset_investments', data, callback));
  socket.on('reset_production', (data, callback) => safeHandler(io, socket, adminHandlers.resetProduction, 'reset_production', data, callback));
...
[서버 종료] ${signal} 신호 수신. 우아한 종료를 시작합니다.`);
  ...
  io.close(() => {
    logger.info('[서버 종료] Socket.IO 연결이 모두 닫혔습니다.');
  });

  await redisClient.quit();
  logger.info('[서버 종료] Redis 연결이 종료되었습니다.');

  logger.info('[서버 종료] 모든 정리 작업이 완료되었습니다.');
  process.exit(0);
}
...
async function startServer() {
  logger.info('[Server] startServer 함수 진입');
  // Redis 연결을 먼저 기다림
  await initializeRedis();
  logger.info('[Server] Redis 초기화 완료');

  server.listen(PORT, () => {
    logger.info(`
========================================`);
    logger.info(`나는 무역왕이 될거야! (Redis-powered)`);
    logger.info(`========================================`);
    logger.info(`포트: ${PORT}`);
    logger.info(`학생용 주소: http://localhost:${PORT}`);
    logger.info(`관리자용 주소: http://localhost:${PORT}/admin`);
    logger.info(`========================================
`);
  });
}
...
// 서버 시작
logger.info('[Server] 스크립트 마지막 줄, startServer 호출 직전');
startServer();